...
llm_models <- c(
  "databricks-claude-sonnet-4-6", # Strong reasoning and comparison of multiple inputs, best overall quality/cost balance
...
) # Slightly older Sonnet model, lower quality than 4.6 but still a strong option
...
Your task is to identify the publication that is most relevant
to the user's data request.

Available publications are provided below as JSON. Each publication
...
{
  \"publication_id\": \"the ID of the best matching publication\"
}
...
call_llm_with_fallback <- function(
  user_input = user_request,
  system_prompt = system_request,
    ...
    result <- tryCatch(
      {
        response <- httr::POST(
          url,
          httr::add_headers(
            Authorization = paste("Bearer", Sys.getenv("DATABRICKS_TOKEN"))
          ),
          encode = "json",
          body = body
        )
